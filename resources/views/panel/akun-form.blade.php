<x-app-layout>
    ...
    <form method="POST" action="{{ $user ? route('panel.akun.update', $user) : route('panel.akun.store') }}" class="bg-white rounded-xl shadow-sm border p-6 max-w-lg space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $user->name ?? '') }}" class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" required>
        </div>
        <div>
            <label class="text-sm text-slate-600">Email</label>
            <input type="email" name="email" value="{{ old('email', $user->email ?? '') }}" class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" required>
        </div>
        <div>
            <label class="text-sm text-slate-600">Role</label>
            <select name="role" class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">
        ...
        <div>
            ...
            <input type="password" name="password" class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" @if(!$user) required @endif>
        </div>
        <div>
            <label class="text-sm text-slate-600">Konfirmasi Password</label>
            <input type="password" name="password_confirmation" class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" @if(!$user) required @endif>
        </div>

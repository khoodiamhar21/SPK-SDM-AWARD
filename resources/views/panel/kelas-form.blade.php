<x-app-layout>
    ...
    <div class="bg-white rounded-xl shadow-sm border p-6 max-w-sm">
        <form method="POST" action="{{ isset($k) ? route('panel.kelas.update', $k) : route('panel.kelas.store') }}">
            ...
            <div class="space-y-4">
                <div><label class="block text-sm font-medium text-slate-700">Nama (I-VI)</label><input name="nama" value="{{ old('nama', $k->nama ?? '') }}" class="w-full rounded-xl border border-slate-300 text-sm mt-1 px-3 py-2"></div>
                <div><label class="block text-sm font-medium text-slate-700">Urutan (1-6)</label><input name="urutan" type="number" value="{{ old('urutan', $k->urutan ?? '') }}" class="w-full rounded-xl border border-slate-300 text-sm mt-1 px-3 py-2"></div>
            </div>
